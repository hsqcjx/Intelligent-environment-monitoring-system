...
/*
	函数功能：设置外设中断的优先级
	函数参数：
					IRQn_Type：           		中断线编号  在中文参考手册的9.1.2 中断向量的位置
					uint32_t PriorityGroup:		对优先级分组
					uint32_t PreemptPriority：主优先级（抢占优先级）
...
{
	OSIntEnter(); //开头添加 OSIntEnter  告诉UCOS操作系统当前程序进入硬件中断
  OSTimeTick(); //时间节拍记录
  OSIntExit();  //结尾添加 OSIntExit   退出中断
}
